...
#sagar shah
        ...
        while True:  # todo: with locks instead
            ...
            if req_id not in self._responses:
                time.sleep(0.025)
                continue
            for index, msg in enumerate(self._responses[req_id]):
                response_data = self._responses[req_id][0].payload
                del self._responses[req_id][0]
                break
        return bytearray(response_data)
    ...
    def _on_client_receiving_response_handler(
       self,
        ...
        :return: None
        """
        print("on_client_receiving_response_handler")
    ...
    def _send_request(self, message) -> UStatus:
        ...
        payload_data = bytearray(message.payload)
        try:
            instance.on_event(method_id, self._on_client_receiving_response_handler) # handles returning a response of data  added by Sagar
            ...
            print(f"error: {ex}")
            return UStatus(message=str(ex), code=UCode.UNKNOWN)
        print("sending request successful\n")
    ...
    def _send_response(self, message) -> UStatus:
        ...
        try:
            ...
            return UStatus(message=str(ex), code=UCode.UNKNOWN)
        print("sending response successful\n")
    ...
    def _register_response_listener(self, source_filter, listener, sink_filter) -> UStatus:
        ...
        try:
            ...
            with self._response_lock:
               ...
               # sagar for resource_id in range(0, 0xFF):  # todo: should register to a range? as its wildcard?
